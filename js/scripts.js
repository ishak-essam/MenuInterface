		...
		function initOnFirstLoad() {
			...
			$('body').waitForImages({
				finished: function () {
					...
					gsap.to($(".preloader-intro"), { duration: 10, fopacity: 1, y: -400, delay: 0.2, ease: Power2.easeInOut });
		...
		if (!$("body").hasClass("disable-ajaxload")) {
						...
						onEnter: function (st) {
							...
							$this.delay(500).queue(function () {
								...
								gsap.to($(".next-hero-progress span"), {
									duration: 0.3, width: "0%", ease: Power2.easeInOut, onComplete: function () {
			...
			$('.next-ajax-link-project').on('click', function () {
				...
				gsap.to($(".next-project-image"), {
					duration: 0.5, scale: 1, opacity: 1, ease: Power2.easeInOut, onComplete: function () {
					...
					setTimeout(function () {
						...
						setTimeout(function () {
							gsap.to($(".next-project-image-wrapper.temporary"), {
								duration: 0.3, opacity: 0, ease: Power2.easeOut, onComplete: function () {
						...
						$('.showcase-pagination').find('.swiper-pagination-bullet').each(function () {
							if (!$(this).hasClass("swiper-pagination-bullet-active")) {
								...
								gsap.to($(this).find('.translate-element span'), {
									duration: 0.5, y: 120, opacity: 0, stagger: -0.02, ease: Power2.easeInOut, onComplete: function () {
					...
					$('.portfolio-wrap').waitForImages({
						finished: function () {
							gsap.utils.toArray('.vertical-parallax').forEach((parallaxElement, index) => {
								...
								gsap.fromTo(parallaxElementChild, { y: offsetParallax * 0.3 }, {
									y: -offsetParallax,
									ease: "none",
									scrollTrigger: {
					...
					$('.portfolio-wrap').waitForImages({
						finished: function () {
							gsap.utils.toArray('.item').forEach((parallaxElement, index) => {
								...
								gsap.fromTo(parallaxElementChild, { y: startMovement }, {
									y: endMovement,
									ease: "none",
									scrollTrigger: {
										trigger: parallaxElement,
					...
					$('.portfolio-wrap').waitForImages({
						finished: function () {
							gsap.utils.toArray('.item').forEach((parallaxElement, index) => {
								...
								gsap.fromTo(parallaxElementChild, { y: 0 }, {
									y: endMovement, scale: 0.5, opacity: 1,
									ease: "none",
									scrollTrigger: {
										trigger: parallaxElement,
										scrub: true,
